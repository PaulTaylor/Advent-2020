import re, sys

def check_valid(passport):
    required_fields = [
        "byr", #(Birth Year)
        "iyr", #(Issue Year)
        "eyr", #(Expiration Year)
        "hgt", #(Height)
        "hcl", #(Hair Color)
        "ecl", #(Eye Color)
        "pid", #(Passp
    ]

    try:
        for f in required_fields:
            passport[f]
    except KeyError:
        return False

    return True

height_pattern = re.compile("^([0-9]+)((cm)|(in))$")
hc_pattern = re.compile("^#[0-9a-f]{6}$")
pid_pattern = re.compile("^[0-9]{9}$")

def check_height(val):
    """(Height) - a number followed by either cm or in:
        If cm, the number must be at least 150 and at most 193.
        If in, the number must be at least 59 and at most 76."""

    match = height_pattern.match(val)
    if not match:
        return False

    num, unit = match.groups()[0:2]
    if unit == 'cm':
        return 150 <= int(num) <= 193
    elif unit == "in":
        return 56 <= int(num) <= 76
    else:
        return False

def check_valid_strict(passport):
    required_fields = {
        "byr": lambda x: (len(x) == 4) and (1920 <= int(x) <= 2002), # four digits; at least 1920 and at most 2002.
        "iyr": lambda x: (len(x) == 4) and (2010 <= int(x) <= 2020), # four digits; at least 2010 and at most 2020.
        "eyr": lambda x: (len(x) == 4) and (2020 <= int(x) <= 2030), # four digits; at least 2020 and at most 2030.
        # height
        #(Height) - a number followed by either cm or in:
        #  - If cm, the number must be at least 150 and at most 193.
        #  - If in, the number must be at least 59 and at most 76.
        "hgt": check_height, # (Height)
        "hcl": lambda x: hc_pattern.match(x), #(Hair Color)
        "ecl": lambda x: x in 'amb blu brn gry grn hzl oth'.split(" "), #(Eye Color)
        "pid": lambda x: pid_pattern.match(x), #(Passp
    }

    try:
        for f, check in required_fields.items():
            assert check(passport[f]), f"field {f} with value {passport.get(f, 'None')} fails check"
    except AssertionError as e:
        #raise e
        return False
    except KeyError:
        return False
    except Exception as e:
        raise e

    return True

if __name__ == "__main__":
    lines = sys.stdin.readlines()

    passports = []
    this_passport = {}
    for line in lines:
        if line.strip() == "":
            # Hit a blank - store the current and reset for next
            passports.append(this_passport)
            this_passport = {}
            continue
        else:
            kv_pairs = line.split(" ")
            for pair in kv_pairs:
                assert len(pair.split(":")) == 2, pair
                k, v = pair.split(":")
                this_passport[k] = v.strip()

    passports.append(this_passport) # deal with the end of the file :)

    validity = [check_valid(p) for p in passports]
    print(f"{sum(validity)} passports are valid\n==========================================")

    validity = [check_valid_strict(p) for p in passports]
    print(f"{sum(validity)} passports are valid with validation rules")